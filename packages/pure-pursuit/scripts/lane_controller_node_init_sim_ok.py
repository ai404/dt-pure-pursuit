...
class lane_controller(object):
    ...
    def update_orientation(self, lane_pose_msg):
        yaw_quat = tf.transformations.quaternion_about_axis(-lane_pose_msg.phi,[0,0,1])
        self.pos_orientation = yaw_quat[:2]
        #rospy.loginfo("[%s] Pos Orientation: %s"%(self.node_name, self.pos_orientation))
    ...
    def update_pose(self, seg_list_msg):
        ...
        def get_tengent(points):
            if len(points["x"])>1:
                idx_max = points["x"].index(max(points["x"]))
                idx_min = points["x"].index(min(points["x"]))
                x_t, y_t = points["x"][idx_max] - points["x"][idx_min],  points["y"][idx_max] - points["y"][idx_min]
                norm_t = math.sqrt(x_t**2 +y_t**2)
                return x_t/norm_t, y_t/norm_t
            else:
                return 0, 0
        ...
        for seg in seg_list_msg.segments:
            ...
            segments[seg.color]["x"].append(x1)
            segments[seg.color]["x"].append(x2)
            segments[seg.color]["y"].append(y1)
